function SlicePlane( volume, p )
{
    ...
    for ( var z = 0; z < volume.resolution.z - 1; z++ )
    {
		...
		var points = [x, y, z];
                ...
                var index = table_index( indices, points );
                if ( index == 0 ) { continue; }
    ...
    function table_index( indices, points )
    {
	...
	/*var s0 = func_plane(index_to_point(indices[0]));
	var s1 = func_plane(index_to_point(indices[1]));
	var s2 = func_plane(index_to_point(indices[2]));
	var s3 = func_plane(index_to_point(indices[3]));
	var s4 = func_plane(index_to_point(indices[4]));
	var s5 = func_plane(index_to_point(indices[5]));
	var s6 = func_plane(index_to_point(indices[6]));
	var s7 = func_plane(index_to_point(indices[7]));*/
    ...
    function index_to_point( index_n )
    {
	var lines = volume.resolution.x;
        var slices = volume.resolution.x * volume.resolution.y;
	var z = Math.floor(index_n / slices);
	index_n -= z*slices;
	var y = Math.floor(index_n / lines);
	index_n -= y*lines;
	var x = index_n;
	return new THREE.Vector3(x, y, z);
    }

    function func_plane( v )
    {
